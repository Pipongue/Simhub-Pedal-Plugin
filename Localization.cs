...
namespace PneumaticCalibratorSimHub
{
    ...
    public static class Localization
    {
        ...
        private static readonly Dictionary<string, (string Fr, string En)> Strings = new Dictionary<string, (string Fr, string En)>
        {
            ...
            ["Flash.WiringCustomAssignment"] = ("Si tu as câblé tes capteurs différemment de ce schéma, désactive \"Assignation par défaut\" dans l'onglet Réglages : un menu déroulant apparaît alors sur chaque canal dans l'onglet Calibration pour choisir manuellement quelle fonction afficher et calibrer. Attention : cela ne change que l'étiquette affichée, pas l'axe réellement piloté dans le jeu (toujours fixé par le câblage physique ci-dessus).",
                                                  "If you wired your sensors differently from this diagram, disable \"Default assignment\" in the Settings tab: a dropdown then appears on each channel in the Calibration tab to manually choose which function to display and calibrate. Note: this only changes the displayed label, not the axis actually driven in-game (still fixed by the physical wiring above)."),
